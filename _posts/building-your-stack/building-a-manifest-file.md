...
<h2 id="environment_variables">Test experimental features</h2>
You can use some features that are still in beta by adding them to 'experiments' section of your manifest.
...
These are the parameters that the <i>experiments</i> section can take for now:

- **install_kernel_318** (_Optional_): If set to "true", we will upgrade new server's linux kernel to latest 3.18 .
- **docker_storage** (_Optional_): If set to 'overlay', we are going to configure docker on new servers with OverlayFS backend storage.
